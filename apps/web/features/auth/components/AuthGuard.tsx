...
export function AuthGuard({ children }: { children: React.ReactNode }) {
    ...
    }, [isReady, sessionChecked, isAuthenticated, user, pathname, router, isPublic, isAuthRoute]);

    // Public marketing and demo routes do not depend on an auth session. Render
    // them immediately so the landing page never sits behind a hydration splash.
    if (isPublic && !isAuthRoute) {
        return <>{children}</>;
    }

    // Protected and auth routes wait for hydration to prevent a redirect flash.
