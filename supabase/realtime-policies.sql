...
--
-- NOTE: these are a starting point. The membership checks below assume the
-- Supabase auth user id is stored on `public.users` (add a column mapping the
-- Supabase user id to your app user if you adopt private channels) — wire the
-- joins to your schema before enabling in production.
    ...
    case
      when realtime.topic() like 'channel:%' then exists (
        ...
        where c.id = split_part(realtime.topic(), ':', 2)
          and u.id = (auth.jwt() ->> 'app_user_id') -- map to your app user id
      )
      -- dm:<conversationId>
      when realtime.topic() like 'dm:%' then exists (
        ...
        where p.conversation_id = split_part(realtime.topic(), ':', 2)
          and u.id = (auth.jwt() ->> 'app_user_id')
      )
      -- user:<userId> — only the user themselves
      when realtime.topic() like 'user:%' then
        split_part(realtime.topic(), ':', 2) = (auth.jwt() ->> 'app_user_id')
      else false
    end
  )
);
...
with check (
  realtime.messages.extension = 'broadcast'
  and (realtime.topic() like 'channel:%' or realtime.topic() like 'dm:%')
);
